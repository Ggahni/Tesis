    ...
    if len(texts) == len(p_texts):
        ...
        if caso == 'diferencia'.upper():
            ...
            for i in range(0, len(bloques_con_cambios)):
                origin_x_value = bloques_con_cambios[i].location.get('x')
                width_x_value  = bloques_con_cambios[i].size.get('width')
                x_value = (width_x_value / 2) + origin_x_value
                if site_x_axis[0] <= x_value < site_x_axis[1]: indice_x = 0
                elif site_x_axis[1] <= x_value < site_x_axis[2]: indice_x = 1
                elif site_x_axis[2] <= x_value < site_x_axis[3]: indice_x = 2
                elif site_x_axis[3] <= x_value < site_x_axis[4]: indice_x = 3
                elif site_x_axis[4] <= x_value < site_x_axis[5]: indice_x = 4
                origin_y_value = bloques_con_cambios[i].location.get('y')
                height_y_value = bloques_con_cambios[i].size.get('height')
                y_value = (height_y_value / 2) + origin_y_value
                if site_y_axis[0] <= y_value < site_y_axis[1]: indice_y = 0
                elif site_y_axis[1] <= y_value < site_y_axis[2]: indice_y = 1
                elif site_y_axis[2] <= y_value < site_y_axis[3]: indice_y = 2
                elif site_y_axis[3] <= y_value < site_y_axis[4]: indice_y = 3
                elif site_y_axis[4] <= y_value < site_y_axis[5]: indice_y = 4

                try:
                    up_left =  block_change_matrix[indice_x-1][indice_y-1]
                except IndexError:
                    up_left = 0

                try:
                    up = block_change_matrix[indice_x][indice_y-1]
                except IndexError:
                    up = 0

                try:
                    up_right = block_change_matrix[indice_x+1][indice_y-1]
                except IndexError:
                    up_right = 0

                try:
                    left = block_change_matrix[indice_x-1][indice_y]
                except IndexError:
                    left = 0

                try:
                    right = block_change_matrix[indice_x+1][indice_y]
                except IndexError:
                    right = 0
                try:
                    down_left = block_change_matrix[indice_x-1][indice_y+1]
                except IndexError:
                    down_left = 0

                try:
                    down = block_change_matrix[indice_x][indice_y+1]
                except IndexError:
                    down = 0

                try:
                    down_right = block_change_matrix[indice_x+1][indice_y+1]
                except IndexError:
                    down_right = 0

                # print('Valores del bloque web son:\n '+'X: '+str(object=origin_x_value)+', Y: '+str(object=origin_y_value)+'\n'+\
                #       'Ancho: '+str(object=width_x_value)+', Alto: '+str(object=height_y_value))
                # print(site_x_axis)
                # print(site_y_axis)
                # print('Coordenadas centro bloque cambio son: ('+str(object=x_value)+','+str(object=y_value)+')')
                # print(str(object=up_left)+' , '+str(object=up)+' , '+str(object=up_right))
                # print(str(object=left)+' , '+str(object=block_change_matrix[indice_x][indice_y])+' , '+str(object=right))
                # print(str(object=down_left)+' , '+str(object=down)+' , '+str(object=down_right))
                cambio_cualitativo   = up_left + up + up_right + \
                                       left + block_change_matrix[indice_x][indice_y] + right + \
